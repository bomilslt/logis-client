...
class TrackingProgress extends HTMLElement {
    ...
    _renderStatusBadge(isException) {
        ...
        return `
            <div class="status-badge ${statusClass}">
                ${ICONS[isException ? 'shield-check' : this.steps[this.getCurrentStepIndex()]?.icon] || ''}
                <span>${label}</span>
            </div>
        `;
    }
    ...
    _calculateProgress(currentIndex) {
        ...
        // Ajuster pour que la ligne s'arrête au centre de l'icône actuelle
        const stepWidth = 100 / totalSteps;
        return `calc(${progress}% - ${stepWidth / 2}%)`;
    }

    _getStepDate(stepId) {
        // Les dates peuvent être passées via un attribut data-dates
    ...
    // Méthode pour mettre à jour le statut dynamiquement
    updateStatus(newStatus) {
        this.setAttribute('status', newStatus);
    }

    // Méthode pour définir les dates
    setDates(dates) {
